...
st.set_page_config(layout="wide", page_title="Finance Model Implementation Demo", page_icon=icon)
...
with rc:
    ...
    with col1:
        st.subheader("💹Finance Model Implementation Demo" + (" (中文)" if use_cn else " (English)"))

    if use_cn:
        st.video("videos/qlib_model_cn.mp4")
    else:
        st.video("videos/qlib_model_en.mov")