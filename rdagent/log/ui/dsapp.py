...
def all_summarize_win():
    ...
    base_df = pd.DataFrame(
        columns=["Competition", "Total Loops", "Successful Final Decision", "Made Submission", "Valid Submission", "Above Median", "Bronze", "Silver", "Gold", "Any Medal"],
        index=summary.keys(),
    )
    for k, v in summary.items():
        ...
        if loop_num == 0:
            ...
            base_df.loc[k, "Valid Submission"] = f"{v['valid_submission_num']} ({round(v['valid_submission_num'] / loop_num * 100, 2)}%)"
            base_df.loc[k, "Above Median"] = f"{v['above_median_num']} ({round(v['above_median_num'] / loop_num * 100, 2)}%)"
