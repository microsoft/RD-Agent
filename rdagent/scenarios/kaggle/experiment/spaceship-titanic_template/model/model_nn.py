...
def fit(X_train, y_train, X_valid, y_valid):
    ...
    train_dataset = TensorDataset(
        torch.tensor(X_train.to_numpy(), dtype=torch.float32), torch.tensor(y_train.to_numpy().reshape(-1), dtype=torch.float32)
    )
    valid_dataset = TensorDataset(
        torch.tensor(X_valid.to_numpy(), dtype=torch.float32), torch.tensor(y_valid.to_numpy().reshape(-1), dtype=torch.float32)
    )
