...
def compute_mae_inspiratory(y_true, y_pred, u_out):  
    """计算吸气阶段的平均绝对误差（MAE）。"""  
...
metrics_all = []
u_out_valid = X_valid['u_out'].values
for model, predict_func, select_m in model_l:
    X_valid_selected = select_m.select(X_valid.copy())
    y_valid_pred = predict_func(model, X_valid_selected)
    mae = compute_mae_inspiratory(y_valid, y_valid_pred, u_out_valid)  
    print(f"MAE on valid set (inspiratory phase): {mae}")  
...
submission_result = pd.DataFrame({"id": ids, "pressure": y_test_pred})
