...
class DSProposalV2ExpGen(ExpGen):
    ...
    def hypothesis_rewrite(
        ...
        time_list = [-3600] + [
            tr[0].running_info.running_time for tr in trace.hist[:-1] if tr[0].running_info.running_time is not None
        ]
        time_max = max(time_list) / 3600
        full_time = self.scen.real_full_timeout() / 3600
        merge_hours = DS_RD_SETTING.merge_hours

        # Filter out None values for successful experiments
        time_list_success = [-3600] + [
            tr[0].running_info.running_time
            for tr in trace.hist[:-1]
            if getattr(tr[1], "decision", False) and tr[0].running_info.running_time is not None
        ]
        if len(time_list_success) > 1 and (max(time_list_success) / 3600) * 1.8 > merge_hours:
    ...
    def hypothesis_smooth(
        ...
        sys_prompt = T(".prompts_v2:hypothesis_smooth.system").r(
            ...
            use_ratio=use_ratio,
            hypothesis_output_format=T(".prompts_v2:output_format.hypothesis_smooth_format").r(
                hypothesis_candidates=hypothesis_candidates
            ),
        )
    ...
    def gen(
        ...
        if DS_RD_SETTING.smooth_flag and use_ratio > 70:
            response_dict = self.hypothesis_smooth(
                scenario_desc=scenario_desc,
                exp_feedback_list_desc=exp_feedback_list_desc,
                sota_exp_desc=sota_exp_desc,
