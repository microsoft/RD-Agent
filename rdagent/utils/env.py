...
class DockerEnv(Env[DockerConf]):
    ...
    def prepare(self):
        ...
        if self.conf.build_from_dockerfile and self.conf.dockerfile_folder_path.exists():
            ...
            resp_stream = client.api.build(path=str(self.conf.dockerfile_folder_path), tag=self.conf.image, network_mode=self.conf.network)
        ...
        try:
            ...
            with Progress(
                ...
                for line in image_pull:
                    ...
                    sp.update(status_task, description=f"[bright_magenta]layer {layer_id} [yellow]{status}", progress=current_status)
