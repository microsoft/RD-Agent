...
class CheckpointSelector:
    ...
    """
    @abstractmethod
    def get_selection(trace: Trace) -> tuple[int, ...]:
        """
        checkpoint_idx represents the place where we want to create a new node.
        - `-1` represents starting from scratch
        - `None` represents starting from the latest trial in the trace.
        """
...
class ExpGen(ABC):
    ...
    @abstractmethod
    def gen(self, trace: Trace, selection: tuple[int, ...] = (,)) -> Experiment:
        """
