...
class TransformerEncoder(Module):
    ...
    def __init__(
        self,
        encoder_layer: "TransformerEncoderLayer" = TransformerEncoderLayer(d_model=512, nhead=8),
        num_layers: int = 6,
...
model_cls = TransformerEncoder


if __name__ == "__main__":
    
    embedding = torch.nn.Embedding(num_embeddings=100, embedding_dim=512)
    test_indices = torch.randint(0, 100, (32, 10))  # 原始索引输入
    test_input = embedding(test_indices)  # 转换为[32,10,512]

    # 或者也可以直接生成emb后的输入
    # test_input = torch.randn(32, 10, 512)  # 形状为 [batch_size, seq_len, d_model]
    
    
        
    # Model instantiation and forward pass
    # model = model_cls(vocab_size=100, max_len=10, hidden_size=100, sos_id=0, eos_id=1)
    model = model_cls(
