...
from rdagent.components.coder.feature_coder.CoSTEER.evaluators import (
...
from rdagent.components.coder.feature_coder.CoSTEER.evolving_agent import (
    FactorRAGEvoAgent,
)
from rdagent.components.coder.feature_coder.CoSTEER.evolving_strategy import (
    FactorEvolvingStrategyWithGraph,
)
from rdagent.components.coder.feature_coder.CoSTEER.knowledge_management import (
    FactorGraphKnowledgeBase,
    FactorGraphRAGStrategy,
    FactorKnowledgeBaseV1,
)
from rdagent.components.coder.feature_coder.factor import FactorExperiment
...
class FeatureCoSTEER(Developer[FactorExperiment]):
    def __init__(
        self,
        *args,
        ...
        **kwargs,
        ...
        self.max_loop = FEATURE_IMPLEMENT_SETTINGS.max_loop
        self.knowledge_base_path = (
            Path(FEATURE_IMPLEMENT_SETTINGS.knowledge_base_path)
            if FEATURE_IMPLEMENT_SETTINGS.knowledge_base_path is not None
            else None
        )
        self.new_knowledge_base_path = (
            Path(FEATURE_IMPLEMENT_SETTINGS.new_knowledge_base_path)
            if FEATURE_IMPLEMENT_SETTINGS.new_knowledge_base_path is not None
            else None
        )
