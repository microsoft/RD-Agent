...
from rdagent.components.coder.feature_coder.CoSTEER.scheduler import (
    LLMSelect,
    RandomSelect,
)
from rdagent.components.coder.feature_coder.factor import FactorFBWorkspace, FactorTask
...
class MultiProcessEvolvingStrategy(EvolvingStrategy):
    ...
    def evolve(
        ...
        **kwargs,
        ...
        if FEATURE_IMPLEMENT_SETTINGS.select_threshold < len(to_be_finished_task_index):
            # Select a fixed number of factors if the total exceeds the threshold
            if FEATURE_IMPLEMENT_SETTINGS.select_method == "random":
                to_be_finished_task_index = RandomSelect(
                    to_be_finished_task_index,
                    FEATURE_IMPLEMENT_SETTINGS.select_threshold,
                )

            if FEATURE_IMPLEMENT_SETTINGS.select_method == "scheduler":
...
class FactorEvolvingStrategy(MultiProcessEvolvingStrategy):
    def implement_one_factor(
        ...
        if queried_knowledge is not None and factor_information_str in queried_knowledge.success_task_to_knowledge_dict:
            ...
            session = APIBackend(use_chat_cache=FEATURE_IMPLEMENT_SETTINGS.coder_use_cache).build_chat_session(
...
class FactorEvolvingStrategyWithGraph(MultiProcessEvolvingStrategy):
    ...
    def implement_one_factor(
        ...
        error_summary = FEATURE_IMPLEMENT_SETTINGS.v2_error_summary
        # 1. 提取因子的背景信息
            ...
            for _ in range(10):  # max attempt to reduce the length of user_prompt
                ...
                if (
                    ...
                    session_summary = APIBackend(
                        use_chat_cache=FEATURE_IMPLEMENT_SETTINGS.coder_use_cache
                    ).build_chat_session(
