...
class FactorFBWorkspace(FBWorkspace):
    ...
    def execute(self, store_result: bool = False, data_type: str = "Debug") -> Tuple[str, pd.DataFrame]:
        ...
        with FileLock(self.workspace_path / "execution.lock"):
            if FEATURE_IMPLEMENT_SETTINGS.enable_execution_cache:
                # NOTE: cache the result for the same code and same data type
                target_file_name = md5_hash(data_type + self.code_dict["factor.py"])
                cache_file_path = Path(FEATURE_IMPLEMENT_SETTINGS.cache_location) / f"{target_file_name}.pkl"
                Path(FEATURE_IMPLEMENT_SETTINGS.cache_location).mkdir(exist_ok=True, parents=True)
            ...
            source_data_path = (
                Path(
                    FEATURE_IMPLEMENT_SETTINGS.data_folder,
                )
                if data_type == "Debug"
                else Path(
                    FEATURE_IMPLEMENT_SETTINGS.data_folder,
            ...
            try:
                subprocess.check_output(
                    f"{FEATURE_IMPLEMENT_SETTINGS.python_bin} {code_path}",
                    shell=True,
                    cwd=self.workspace_path,
                    stderr=subprocess.STDOUT,
                    timeout=FEATURE_IMPLEMENT_SETTINGS.file_based_execution_timeout,
                )
                ...
                execution_feedback += f"Execution timeout error and the timeout is set to {FEATURE_IMPLEMENT_SETTINGS.file_based_execution_timeout} seconds."
                if self.raise_exception:
                    raise CustomRuntimeError(execution_feedback)
