...
def prepare_notebooks(competitions, notebook_path, idea_path, max_num=5):
    ...
    for competition in tqdm(competitions, desc="Extracting Ideas"):
        desc_file = f"{notebook_path}/{competition}.json"
        if not os.path.exists(desc_file):
            continue
        competition_desc = load_description(f"{notebook_path}/{competition}.json")

        nb_path = f"{notebook_path}/notebooks(top5+median5)/{competition}/best"
        for root, dirs, files in os.walk(nb_path):
            for file in files:
                if file.endswith(".ipynb"):
                    ...
                    for idea, idea_content in new_ideas.items():
                        new_ideas[idea]['competition'] = competition
...
if __name__ == "__main__":
    notebook_path = "/data/userdata/v-zhangyifei/minrui/RD-Agent/Notebook"
    competitions = os.listdir(f"{notebook_path}/notebooks(top5+median5)")

    prepare_notebooks(competitions=competitions, 
                      notebook_path=notebook_path,
                      idea_path="/data/userdata/v-xuminrui/RD-Agent/knowledge/idea_test_v4.json")