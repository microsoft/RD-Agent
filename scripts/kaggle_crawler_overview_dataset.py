...
class KaggleCompetitionInfo(BaseModel):
    ...
    tags: list[str] = Field(..., description="Tags of the competition，should give the full tag list, not just the first few tags.")
...
async def crawl_page_with_retry(crawler: AsyncWebCrawler, url: str, config: CrawlerRunConfig, page_type: str, competition_name: str) -> Optional[Dict[str, Any]]:
    """带重试机制的页面爬取函数"""
    retry_count = 0
    last_error = None
...
async def main():
    ...
    df = pd.read_csv('/data/userdata/v-zhangyifei/MSRA/github/knowledge/github_competitions/比赛小节/competitions_extracted_info.csv')
    competition_list = df['link_slug'].tolist()
    print(f"处理{len(competition_list)}个比赛")
    
    # 设置自定义保存路径（可以修改为你想要的路径）
    custom_save_path = "/data/userdata/v-zhangyifei/MSRA/github/knowledge/kaggle_competitions"  # 设置为None则使用当前目录
