...
def get_loop_idx(log_trace_path):
    """
    Determines the early-stage and late-stage indices from a session log trace directory.
    
    Parameters:
        log_trace_path (str): The path to the log trace directory containing the '__session__' folder.

    Returns:
        tuple: A tuple containing two integers:
            - es_loop (int): The early-stage loop index where all components are complete.
            - ls_loop (int): The late-stage (last) loop index found in the session. 
    """
...
def get_last_step(session_path):
    """
    Determines last step within a session path.
    
    Parameters:
        session_path (str): Path like LOG_TRACE_PATH/__session__.

    Returns:
        step (str): Last step within the session path (such as '4_record').
    """
...
def main():
    ...
    # list the competitions for running
